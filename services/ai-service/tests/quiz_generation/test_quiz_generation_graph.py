...
def test_quiz_generation_graph_runner(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.workflows.quiz_generation.nodes.load_inputs.QuizGenerationInputService.load_profile_context",
    ...
    def fake_publish(_self, **kwargs):
        publish_kwargs.update(kwargs)
        return [CreatedQuizQuestionRead(questionId=1001, questionUuid="qq-1", sortOrder=5)]
    ...
    assert publish_kwargs["purpose"] == "authoring"


def test_quiz_generation_graph_uses_attempt_purpose_for_learner_runs(monkeypatch):
    publish_kwargs = {}
    monkeypatch.setattr(
        "app.services.workflows.quiz_generation.nodes.load_inputs.QuizGenerationInputService.load_context",
        lambda self, payload: QuizGenerationContextRead(
    ...
    QuizGenerationGraphRunner(session=object()).run(
        payload=QuizGenerationRequest(
            courseUuid="course-uuid",
            moduleUuid="module-uuid",
            educatorId=7,
    ...
    assert publish_kwargs["purpose"] == "attempt"
